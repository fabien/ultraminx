...
Ultraminx is not an object-relational mapper, and the association generation is intended to stay minimal--don't be afraid of <tt>:association_sql</tt>.
...
Note that the column type must be time-comparable in the DB. Also note that faceting may return higher counts than actually exist on delta-indexed tables, and that sorting by string columns will not work well. These are both limitations of Sphinx's index merge scheme. You can perhaps mitigate the issues by only searching the main index for facets or sorts:
  ...
  Ultraminx::Search.new(:query => "query", :indexes => Ultraminx::MAIN_INDEX)
...
A common use case is to only search records that belong to a particular parent model. Ultraminx configures Sphinx to support a <tt>:filters</tt> element on any date or numeric field, so any <tt>*_id</tt> fields you have will be filterable.
...
Now you can run:
 ...
 @search = Ultraminx::Search.new('something', 
...
=end
    ...
    VALID_FIND_OPTIONS = [:conditions, :include, :group, :order, :limit, :offset, :joins, :select, :from, :readonly, :lock]
    ...
    def self.is_indexed opts = {}    
      ...
      opts['concatenate'].each do |entry|
        ...
        raise Ultraminx::ConfigurationError, "You can't mix regular concat and group concats" if entry['fields'] and (entry['field'] or entry['class_name'] or entry['association_name'])
        raise Ultraminx::ConfigurationError, "Concatenations must specify an :as key" unless entry['as']
        raise Ultraminx::ConfigurationError, "Group concatenations must not have multiple fields" if entry['field'].is_a? Array
        raise Ultraminx::ConfigurationError, "Regular concatenations should have multiple fields" if entry['fields'] and !entry['fields'].is_a?(Array)
      end
      ...
      Ultraminx::MODEL_CONFIGURATION[self.name] = opts
    end
    
    def self.search(*args)
      options = args.extract_options!
      options[:query] ||= args.first.to_s unless args.first.blank?
      if options[:excerpt] || options[:excerpts]
        result = ultraminx_search(options.except(:excerpt, :excerpts)).excerpt rescue []
      else
        result_ids = ultraminx_search(options).run(false).results.map(&:last).uniq rescue []
        return [] if result_ids.empty?
        result = with_scope(:find => { :conditions => { :id => result_ids } }) { find(:all, options.slice(*VALID_FIND_OPTIONS)) }
        if options[:order].blank? # Sphinx ordered results
          result.sort! { |a, b| result_ids.index(a.id) <=> result_ids.index(b.id) }
        end
      end
      result
    end
    
    def self.ultraminx_search(*args)
      options = args.extract_options!
      options[:query] ||= args.first.to_s unless args.first.blank?
      options[:class_names] = ([self] + self.subclasses + Array(options[:class_names] || [])).uniq.map(&:name)
      Ultraminx::Search.new(options.except(*VALID_FIND_OPTIONS))
    end

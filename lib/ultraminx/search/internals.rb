    ...
    module Internals
      ...
      def build_request_with_options opts
        ...
        request.instance_eval do          
          @server = Ultraminx::CLIENT_SETTINGS['server_host']
          @port = Ultraminx::CLIENT_SETTINGS['server_port']          
          @match_mode = :extended # Force extended query mode
          @offset = opts['per_page'] * (opts['page'] - 1)
          @limit = opts['per_page']
          @max_matches = [@offset + @limit + Ultraminx::Search.client_options['max_matches_offset'], MAX_MATCHES].min
        end
      ...
      def get_facets(original_request, query, original_facet)
        ...
        request.instance_eval do
          ...
          @offset = 0
          @match_mode = Ultraminx::Search.client_options['match_mode']
          @limit = Ultraminx::Search.client_options['max_facets']
          @max_matches = [@limit + Ultraminx::Search.client_options['max_matches_offset'], MAX_MATCHES].min
        end
        ...
        begin
          ...
          raise ConfigurationError, "Index seems out of date. Run 'rake ultraminx:index'"
        end
      ...
      def rebuild_facet_cache(facet)
        ...
        Ultraminx.say "caching hash reverse map for text facet #{facet}"
        ...
        configured_classes = Fields.instance.classes[facet].map do |klass|
          ...
          unless configuration and configuration['facet']
            Ultraminx.say "model #{klass.name} has the requested '#{facet}' field, but it was not configured for faceting, and will be skipped"
            next
          end
      ...
      def reify_results(ids)
        ...
        ids.map {|ary| ary.first}.uniq.each do |class_name|
          ...
          finder = (
            Ultraminx::Search.client_options['finder_methods'].detect do |method_name| 
          ...
          unless Ultraminx::Search.client_options['ignore_missing_records']
        ...
        if ids.size - results.size > Ultraminx::Search.client_options['max_missing_records']
          # Never reached if Ultraminx::Search.client_options['ignore_missing_records'] is false due to raise
      ...
      def perform_action_with_retries
        ...
        begin
          yield
            ...
            Errno::EPIPE => e
          tries += 1
          if tries <= Ultraminx::Search.client_options['max_retries']
            say "restarting query (#{tries} attempts already) (#{e})"            
            sleep(Ultraminx::Search.client_options['retry_sleep_time']) 
            retry
          else
            say "query failed"

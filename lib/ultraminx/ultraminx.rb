...
module Ultraminx
  ...
  DIR = File.join(Merb.dir_for(:config), 'ultraminx')
  
  THIS_DIR = File.expand_path(File.dirname(__FILE__))

  CONF_PATH = "#{DIR}/#{Merb.environment}.conf"
  
  ENV_BASE_PATH = "#{DIR}/#{Merb.environment}.base" 
  
  GENERIC_BASE_PATH = "#{DIR}/default.base"
  
  BASE_PATH = (File.exist?(ENV_BASE_PATH) ? ENV_BASE_PATH : GENERIC_BASE_PATH)
  
  raise ConfigurationError, "Please create a '#{DIR}/#{Merb.environment}.base' or '#{DIR}/default.base' file in order to use Ultraminx in your #{Merb.environment} environment." unless File.exist? BASE_PATH # XXX lame
  ...
  CONFIG_MAP = {
    # These must be symbols for key mapping against Merb itself.
    :username => 'sql_user',
    :password => 'sql_pass',
    :host => 'sql_host',
    :database => 'sql_db',
...
sql_query_pre = ) + SQL_FUNCTIONS['postgresql']['stored_procedures'].values.join(' ') + %(
  ...
  def self.options_for(heading, path)
    # Evaluate using Erubis
    template = Erubis::MEruby.new(IO.read(File.expand_path(path)))
    ...
    if section
      ...
      Ultraminx.say "warning; heading #{heading} not found in #{path}; it may be corrupted. "
      {}    
    end    
  end
  ...
  def self.get_models_to_class_ids #:nodoc:
    ...
    if File.exist? CONF_PATH
      ...
      msg = "#{CONF_PATH} file is corrupted. Please run 'rake ultraminx:configure'."
      ...
      Ultraminx.say "configuration file not found for #{Merb.environment} environment"
      Ultraminx.say "please run 'rake ultraminx:configure'"
    end      
  end  
  ...
  STOPWORDS_PATH = "#{Ultraminx::INDEX_SETTINGS['path']}/#{DICTIONARY}-stopwords.txt"
    ...
    if File.exist? CONF_PATH
      begin
        if options_for(
          ...
          say "please run 'rake ultraminx:configure'"
        end 
      rescue Object
      end
    end

  ...
  class Configure  
    class << self
      ...
      def load_constants
        ...
        Dir.chdir "#{Merb.root}/app/models/" do
      ...
      def run       
        ...
        say "rebuilding configurations for #{Merb.environment} environment" 
        ...
        if index == DELTA_INDEX and options['delta']
          ...
          if delta_column 
            ...
            if (options['fields'] + options['concatenate'] + options['include']).detect { |entry| entry['sortable'] }
              ...
              Ultraminx.say "warning; text sortable columns on #{klass.name} will return wrong results with partial delta indexing"
            end            
            string = "#{source_string} > #{SQL_FUNCTIONS[ADAPTER]['delta']._interpolate(INDEXER_SETTINGS['delta'])}";
          else
            Ultraminx.say "warning; #{klass.name} will reindex the entire table during delta indexing"
          end
        end
      end
      ...
      def build_regular_fields(klass, fields, entries, column_strings, join_strings, group_bys, remaining_columns)         
        entries.compact.to_a.each do |entry|

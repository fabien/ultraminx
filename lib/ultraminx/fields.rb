...
=begin rdoc
This is a special singleton configuration class that stores the index field configurations. Rather than using a magic hash and including relevant behavior in Ultraminx::Configure and Ultraminx::Search, we unify it here.
=end
        ...
        begin
          ...
          options['fields'] = options['fields'].to_a.map do |entry|
            ...
            unless klass.columns_hash[entry['field']]
              ...
              Ultraminx.say "warning: field #{entry['field']} is not present in #{model}"
            else
          ...
          Ultraminx.say "warning: model #{model} does not exist in the database yet"
        end  
      end

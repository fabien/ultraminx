...
To set up a search, instantiate an Ultraminx::Search object with a hash of parameters. Only the <tt>:query</tt> key is mandatory.
  @search = Ultraminx::Search.new(
    :query => @query, 
...
<tt>:indexes</tt>:: An array of indexes to search. Currently only <tt>Ultraminx::MAIN_INDEX</tt> and <tt>Ultraminx::DELTA_INDEX</tt> are available. Defaults to both; changing this is rarely needed.
...
Ultraminx uses the <tt>find_all_by_id</tt> method to instantiate records. If you set <tt>with_finders: true</tt> in {Interlock's}[http://blog.evanweaver.com/files/doc/fauna/interlock] <tt>config/memcached.yml</tt>, Interlock overrides <tt>find_all_by_id</tt> with a caching version.
...
There are some other keys you can set, such as excerpt size, HTML tags to highlight with, and number of words on either side of each excerpt chunk. Example (in <tt>environment.rb</tt>):
  ...
  Ultraminx::Search.excerpting_options = HashWithIndifferentAccess.new({
    :before_match => '<strong>', 
    :after_match => '</strong>',
    :chunk_separator => "...",
    :limit => 256,
...
Note that your database is never changed by anything Ultraminx does.
...
=end    
  ...
  class Search  
    ...
    self.query_defaults ||= HashWithIndifferentAccess.new({
      ...
      :indexes => [
          MAIN_INDEX, 
          (DELTA_INDEX if Ultraminx.delta_index_present?)
        ].compact,
      :weights => {},
      :class_names => [],
      :filters => {},
    ...
    MODELS_TO_IDS = Ultraminx.get_models_to_class_ids || {} 
    ...
    # Returns a hash of total result counts, scoped to each available model. Set <tt>Ultraminx::Search.client_options[:with_subtotals] = true</tt> to enable.
    # 
    ...
    def initialize opts = {} 
      ...
      say "discarded invalid keys: #{extra_keys * ', '}" if extra_keys.any? and Merb.environment != "test" 
    end
